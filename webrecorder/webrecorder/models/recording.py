...
class Recording(RedisUniqueComponent):
    ...
    def serialize(self,
        ...
        if include_files:
            files = {}
            files['warcs'] = [
                n for n, v in self.iter_all_files(include_index=False)
            ]
    ...
    def delete_files(self, storage):
        ...
        for n, v in self.iter_all_files(include_index=True):
            ...
            #  may be a local, uncomitted file,
            # that must be deleted with local storage
            if not success:
    ...
    def set_patch_recording(self, patch_recording, update_ts=True):
        ...
        if patch_recording:
            self.set_prop(
                'patch_rec', patch_recording.my_id, update_ts=update_ts
            )
    ...
    def commit_to_storage(self, storage=None):
        ...
        if storage:
            all_done = collection.commit_file(
            ...
            for warc_filename, warc_full_filename in self.iter_all_files():
                done = collection.commit_file(
                    warc_filename, warc_full_filename, 'warcs', warc_key
                )
        ...
        for n, url in source.iter_all_files(include_index=True):
            ...
            try:
                ...
                if n != self.INDEX_FILE_KEY:
                    self.incr_size(size)
                    self.redis.hset(
                        target_warc_key, n, add_local_store_prefix(target_file)
                    )
                else:
