...
cleanup() {
    ...
    # add '.done' file so browser controller script knows we're ready to shutdown!
    touch "/home/browser/.done"
}
...
${WEBSOCKIFY} ${SSLONLY} --web ${WEB} ${CERT:+--cert ${CERT}} ${PORT} ${VNC_DEST} --run-once &
proxy_pid="$!"
sleep 1
