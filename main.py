...
class MultiUserRouter(object):
    ...
    def get_state(kwargs):
        ...
        info = RouteInfo(user + '/' + coll,
                         user,
                         coll,
                         2, './accounts/{0}/collections/{1}/archive'.format(user, coll))
...
class SingleUserRouter(object):
    ...
    def get_state(kwargs):
        ...
        info = RouteInfo(coll,
                        'default',
                         coll,
                         1, './collections/{0}/archive'.format(coll))

        return info



def init(configfile='config.yaml'):
    ...
    global redis_obj
    redis_obj = StrictRedis.from_url(config['redis_url'])
